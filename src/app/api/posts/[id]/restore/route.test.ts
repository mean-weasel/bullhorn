...
describe('POST /api/posts/[id]/restore', () => {
  ...
  it('returns 404 when post not found', async () => {
    ...
    const req = createRequest('/api/posts/nonexistent/restore', { method: 'POST' })
  ...
  it('returns 400 when post is not archived', async () => {
    mockRequireAuth.mockResolvedValue({ userId: 'user-1' })
    mockFetchSingle.mockResolvedValue({
      data: { status: 'draft' },
      error: null,
  ...
  it('restores an archived post successfully', async () => {
    ...
    mockUpdateSingle.mockResolvedValue({ data: restoredPost, error: null })
    const req = createRequest('/api/posts/post-1/restore', { method: 'POST' })
    const res = await POST(req, makeParams('post-1'))
