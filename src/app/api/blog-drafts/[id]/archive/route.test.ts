...
describe('POST /api/blog-drafts/[id]/archive', () => {
  ...
  it('returns 404 when blog draft not found', async () => {
    ...
    const req = createRequest('/api/blog-drafts/nonexistent/archive', { method: 'POST' })
  ...
  it('returns 400 when blog draft is already archived', async () => {
    mockRequireAuth.mockResolvedValue({ userId: 'user-1' })
    mockFetchSingle.mockResolvedValue({
    ...
    const body = await res.json()
    expect(body.error).toBe('Blog draft is already archived')
  })
  ...
  it('archives a blog draft successfully', async () => {
    ...
    mockUpdateSingle.mockResolvedValue({ data: archivedDraft, error: null })
    const req = createRequest('/api/blog-drafts/draft-1/archive', { method: 'POST' })
    const res = await POST(req, makeParams('draft-1'))
  ...
  it('filters by user_id when querying', async () => {
    ...
    expect(mockSelectEq1).toHaveBeenCalledWith('id', 'draft-1')
    expect(mockSelectEq2).toHaveBeenCalledWith('user_id', 'user-1')
    expect(mockUpdateEq1).toHaveBeenCalledWith('id', 'draft-1')
